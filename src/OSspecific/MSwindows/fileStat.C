    ...
    actualValidFileName_()
{
    ...
    // _stat supplies whole seconds. Preserve the last-write fractional part
    // used by highResLastModified instead of reading uninitialised storage.
    if (isValid_)
    {
        WIN32_FILE_ATTRIBUTE_DATA attributes;
        if (GetFileAttributesExA
            (actualValidFileName_.c_str(), GetFileExInfoStandard, &attributes))
        {
            const unsigned long long ticks =
                (static_cast<unsigned long long>
                    (attributes.ftLastWriteTime.dwHighDateTime) << 32)
              | attributes.ftLastWriteTime.dwLowDateTime;
            status_.st_atim.tv_nsec = (ticks % 10000000ULL)*100;
        }
    }
}
